...
var print_results=function(){
	...
	for(i=0; i<results.length; i++){						
						...
						document.getElementById("results").innerHTML+="<span style='font-size:70%;'><a style='color:#04B45F;cursor:pointer;' onClick='add_rel("+i+")'>Relevant</a>&nbsp;<a style='color:#F78181;cursor:pointer;' onClick='rem_rel("+i+")'>Irelevant</a></span>"
...
var add_rel= function (x){
	var exists = irrelevant_docs.indexOf(results[x]);
	if(exists != -1){
		irrelevant_docs.splice(exists, 1);
	}else{
		var inAlready = relevant_docs.indexOf(results[x]);
		if(inAlready == -1){
...
var rem_rel= function (x){
	...
	if(exists == -1){
		var inAlready = irrelevant_docs.indexOf(results[x]);
		if(inAlready == -1){
			irrelevant_docs.push(results[x]);	
		}
	}else{
...
var relevance_print= function(){
	...
	if (relevant_docs.length > 0){
		new_state+="<div class='alert alert-success'>";
		for(var i=0; i<relevant_docs.length; i++){
			new_state= new_state+relevant_docs[i].docTitle+" is relevant.<br/>";
		}
		new_state+="</div>";
	}
	if (irrelevant_docs.length > 0){
		new_state+="<div class='alert alert-danger'>";
		for(var i=0; i<irrelevant_docs.length; i++){
			new_state= new_state+irrelevant_docs[i].docTitle+" is irrelevant.<br/>";
		}
		new_state+="</div>";
	}
	if(new_state.length>0){
		document.getElementById("relevant").innerHTML="<div class='row'><a class='btn btn-success btn-sm' style='float:right' onClick='refine_search()' href='#'>Refine Search</a></div><br>";
...
var refine_search= function(){
	...
	document.getElementById("results").innerHTML="<div class='alert alert-success'>Calculating...</div>";
	res_desc=[];
	new_vector=[];
	for(var i=0; i<query.length; i++){
		new_vector.push(query[i]);
	}
	if(relevant_docs.length > 0){
		for(var j=0; j<relevant_docs.length; j++){
			for(var i=0; i<query.length; i++){
				new_vector[i]= new_vector[i]+(0.5*relevant_docs[j].vect[i]);
			}
		}
	}
	if(irrelevant_docs.length > 0){
		for(var j=0; j<irrelevant_docs.length; j++){
			for(var i=0; i<query.length; i++){
				new_vector[i]= new_vector[i]-(0.5*irrelevant_docs[j].vect[i]);
			}
		}
	}
